...
try {
    ...
    do {
        ...
        $events = @(Get-WinEvent -FilterHashtable @{ LogName='WinFIM-Operational'; ProviderName=$source; StartTime=$start } -ErrorAction SilentlyContinue)
    ...
    $heartbeat = Get-WinEvent -FilterHashtable @{ LogName='WinFIM-Operational'; ProviderName=$source; StartTime=$start; Id=7790 } -ErrorAction SilentlyContinue |
        Select-Object -First 1
